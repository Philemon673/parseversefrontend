...
import { initVideoUpload, uploadVideoToBunny } from "@/lib/courseService";
import { createPost, uploadPostMedia } from "@/lib/postService";
...
function DropZone({ file, onFile, onClear }) {
  ...
  const handleDrop = useCallback(
    (e) => {
      ...
      if (f && (f.type.startsWith("video/") || f.type.startsWith("image/"))) onFile(f);
    },
    [onFile]
  );
    ...
    >
      ...
      <div className="text-center">
        <p className="font-bold text-[#2d2560] text-base">
          {dragging ? "Drop it here!" : "Drag and drop your Video or Image here"}
        </p>
      ...
      <p className="text-xs text-[#9b96c0] text-center">
        MP4, MOV, PNG, JPG, JPEG &nbsp;·&nbsp; Max size 2GB
      </p>
      <input
        ref={inputRef}
        type="file"
...
export default function UploadVideoPage() {
  ...
  const fileMeta = file
    ? {
        duration: file.type.startsWith("video/") ? "Short" : "N/A",
        resolution: "Auto",
  ...
  const handleSaveShort = async () => {
    ...
    try {
      ...
      if (isVideo) {
        ...
        setProgress(15);
        const meta = await initVideoUpload(title);
        setProgress(30);

        // 2. Upload to Bunny via TUS
        await uploadVideoToBunny(file, meta, (percent) => {
          setProgress(30 + Math.round(percent * 0.5));
        });
        ...
        // Upload image to CDN
        setProgress(40);
        const uploadResult = await uploadPostMedia(file, "image");
      ...
      // Parse tags
      const parsedTags = description.match(/#\w+/g)?.map((t) => t.replace("#", "")) || [];
  ...
  return (
    <div className="min-h-screen bg-[#f8f8fb] font-sans pb-12">
      {/* ── Header ── */}
      <div className="bg-white border-b border-[#ede9fd] px-8 pt-7 pb-6">
        <div className="max-w-5xl mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-black text-[#18165a] tracking-tight">Create a Short</h1>
              <p className="text-sm text-[#9490b5] mt-0.5">
                Upload a video or picture short to boost visibility and link to your profile or courses.
              </p>
            </div>
            <button
              onClick={() => router.back()}
      ...
      <main className="max-w-5xl mx-auto px-6 py-10 grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-2 flex flex-col gap-5">
          ...
          >
            <DropZone file={file} onFile={(f) => { setFile(f); setCurrentStep(2); }} onClear={() => setFile(null)} />
          </div>
          ...
          {isUploading && (
            <div className="bg-white rounded-2xl p-5 border border-indigo-100 shadow-sm flex flex-col gap-3">
              ...
              <div className="w-full h-2 bg-indigo-50 rounded-full overflow-hidden">
                <div
                  className="h-full bg-[#4f3fd8] rounded-full transition-all duration-300"
          ...
          {!file && (
            ...
            >
              ...
              Upload a video or image to populate metadata.
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-3 flex flex-col gap-5">
          <div
            ...
            style={{
              border: "1px solid #ddd8fa",
              boxShadow: "0 4px 24px 0 rgba(79,63,216,0.10), 0 1px 4px 0 rgba(79,63,216,0.06)",
            }}
          >
            <div className="flex items-center gap-4 px-7 pt-7 pb-5 border-b border-[#f0edfb]">
              ...
              <div>
                <h2 className="text-xl font-black text-[#18165a] tracking-tight">Short Details</h2>
                <p className="text-sm text-[#9490b5] font-medium">Add descriptive details and a link</p>
              </div>
            </div>
            ...
            <div className="px-7 py-6 flex flex-col gap-6">
              ...
              <div>
                <FieldLabel
                  ...
                  tooltip="Help viewers find your short by including descriptive text and TikTok-style hashtags (e.g., #React #Coding)."
                />
                <div
                ...
                >
                  ...
                  <div className="flex-1 relative">
                    <textarea
                      value={description}
                      onChange={(e) => e.target.value.length <= DESC_MAX && setDescription(e.target.value)}
                      onFocus={() => setFocusedField("desc")}
                      onBlur={() => setFocusedField(null)}
                      placeholder="Add summary and hashtags (e.g. #Programming #Tutorial)..."
                      rows={5}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <SelectField value={language} options={LANGUAGES} onChange={setLanguage} prefixIcon={Globe} />
                </div>
                <div>
                  <FieldLabel label="Visibility" />
                  <SelectField value={visibility} options={VISIBILITIES} onChange={setVisibility} prefixIcon={Lock} />
                </div>
              </div>
            </div>
          </div>
          ...
          <button
            ...
            className="flex items-center justify-center gap-2.5 px-8 w-full py-3 rounded-xl bg-[#4f3fd8] text-white text-sm font-black hover:bg-[#3d2fc0] active:scale-[0.99] transition-all disabled:opacity-50"
            style={{ boxShadow: "0 2px 10px 0 rgba(79,63,216,0.28)" }}
          >
            <Save className="w-4 h-4" />
            {isUploading ? "Uploading..." : "Save Short"}
          </button>
        </div>
      </main>
    </div>
